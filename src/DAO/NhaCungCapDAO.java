...
public class NhaCungCapDAO {
    ...
    public ArrayList<NhaCungCapDTO> getTenNhaCungCap(){
        ArrayList<NhaCungCapDTO> listTenNhaCungCap = new ArrayList<>();
        connection = MySQLConnection.getConnection();
        String query = "SELECT tennhacungcap FROM nhacungcap WHERE trangthai = 1";
        try {
            pst = connection.prepareStatement(query);
            ResultSet rs = pst.executeQuery();
            while (rs.next()) {
                NhaCungCapDTO nhaCungCapDTO = new NhaCungCapDTO();
                nhaCungCapDTO.setTenncc(rs.getString("tennhacungcap"));
                listTenNhaCungCap.add(nhaCungCapDTO);
            }
        } catch (SQLException e) {
            e.printStackTrace();
            JOptionPane.showMessageDialog(null, "Không thể tải dữ liệu nhà cung cấp");
